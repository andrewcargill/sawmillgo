...
const PlankFromLog = ({ formData, setFormData, setShowForm }) => {
  ...
  const fetchTreeData = async (treeRefId) => {
    const treesCollection = collection(db, `sawmill/${sawmillId}/logs`);
  ...
  const updateFormDataWithTreeData = (treeData, logId) => {
    // Construct the new form data object
    const updatedFormData = {
      ...formData,
      logId: treeData.refId,
      verified: true,
  ...
  return (
    ...
    >
      {showTreeId ? (
        <Grid item xs={12}>
          <Typography variant="h6">LOG: {formData.logId} - {formData.speciesName}</Typography>
        </Grid>
      ) : (
        <Grid item xs={12}>
